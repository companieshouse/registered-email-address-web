...
import { InvalidCompanyHandler } from "./handlers/company/invalid_company";
...
import { COMPANY_PROFILE, COMPANY_NUMBER, INVALID_COMPANY_REASON } from "../constants/app_const";
import { INVALID_COMPANY_SERVICE_UNAVAILABLE } from "../constants/validation_const";

const company_router: Router = Router();
const routeViews: string = "router-views/company/";
const invalidCompanyReason: string = "invalidCompanyReason";

// GET: /registered-email-address/company/number
company_router.get(NUMBER_URL, (req: Request, res: Response, next: NextFunction) => {
...
company_router.post(CONFIRM_URL, async (req: Request, res: Response, next: NextFunction) => {
...
company_router.get(INVALID_URL, async (req: Request, res: Response, next: NextFunction) => {
