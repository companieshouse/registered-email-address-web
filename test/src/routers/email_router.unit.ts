...
describe("Email router tests", () => {
        ...
        describe("Change email tests", () => {
            it("Should navigate to change email page", async () => {
                const getSpy = jest
                    ...
                    .mockResolvedValue(clone(okResponse));
            ...
            it("Should re-display change email page when Email not entered", async () => {
                const errorObject = {
                    errors: { changeEmailAddress: "You need to accept the registered email address statement" },
                };
            ...
            it("Should navigate to check your answer page when valid Email entered", async () => {
                const postSpy = jest
                    .spyOn(ChangeEmailAddressHandler.prototype, "post")
                    .mockResolvedValue(movedTemporarilyResponse);
                ...
                await request(app)
                    ...
                    .then(response => {
                        ...
                        expect(response.text).toContain(
                            "Redirecting to /registered-email-address/email/check-your-answer"
                        );
                        expect(postSpy).toHaveBeenCalled();
                    });
            });
    ...
    describe("Update submitted tests", () => {
        ...
        it("Should navigate to update submitted page", async () => {
            const getSpy = jest
                .spyOn(UpdateSubmittedHandler.prototype, "get")
                .mockResolvedValue({ title: "Application submitted" });
        ...
        it("Should navigate back to Confirmation Statement service when originally called from it", async () => {
            const getSpy = jest
                .spyOn(UpdateSubmittedHandler.prototype, "post")
                .mockResolvedValue({ title: "Application submitted" });
