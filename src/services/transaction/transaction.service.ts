...
import { DESCRIPTION, headers, REFERENCE, transactionStatuses } from "../../config/index";
 ...
 */
export const closeTransaction = async (session: Session, companyNumber: string, registeredEmailAddressId: string, transactionId: string): Promise<string | undefined> => {
    const apiResponse: ApiResponse<Transaction> = await putTransaction(session, companyNumber, registeredEmailAddressId, transactionId, DESCRIPTION, transactionStatuses.CLOSED);
...
export const putTransaction = async (session: Session,
    ...
    registeredEmailAddressId: string,
    transactionId: string,
    transactionDescription: string,
    transactionStatus: string): Promise<ApiResponse<Transaction>> => {
    const apiClient: ApiClient = createPublicOAuthApiClient(session);

    const csReference = `${REFERENCE}_${registeredEmailAddressId}`;
    const transaction: Transaction = {
        companyNumber,
