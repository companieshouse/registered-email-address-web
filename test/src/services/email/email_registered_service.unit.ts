...
describe("Registered Email service test", () => {
    ...
    const mockResponseBody: RegisteredEmailAddress = {
    ...
    describe("postRegisteredEmailAddress tests", () => {
        it("Should successfully post a registered email address", async () => {
            ...
            mockPostTransaction.mockResolvedValueOnce(
                mockedResponse as ApiResponse<RegisteredEmailAddressCreatedResource>
            );
            ...
            await postRegisteredEmailAddress(session, TRANSACTION_ID, COMPANY_NUMBER, mockResponseBody).then(data => {
                ...
                const castedData: RegisteredEmailAddressCreatedResource =
                    data.resource as RegisteredEmailAddressCreatedResource;
                expect(castedData.id).toEqual(ID);
                expect(castedData.data.registeredEmailAddress).toEqual(EMAIL_ADDRESS_TO_REGISTER);
                expect(castedData.data.acceptAppropriateEmailAddressStatement).toEqual(
        ...
        it("Should throw an error when no registered email api response", async () => {
            ...
            await expect(
                postRegisteredEmailAddress(session, TRANSACTION_ID, COMPANY_NUMBER, mockResponseBody)
