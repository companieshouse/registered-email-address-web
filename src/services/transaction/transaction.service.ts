...
export const postTransaction = async (session: Session, companyNumber: string, description: string, reference: string): Promise<Transaction> => {
  ...
  if (!sdkResponse) {
    throw createAndLogError( THERE_IS_A_PROBLEM, `Transaction API POST request returned no response for company number ${companyNumber}`);
  }

  if (!sdkResponse.httpStatusCode || sdkResponse.httpStatusCode >= StatusCodes.BAD_REQUEST) {
    throw createAndLogError( THERE_IS_A_PROBLEM, `Http status code ${sdkResponse.httpStatusCode} - Failed to post transaction for company number ${companyNumber}`);
  }

  const castedSdkResponse: Resource<Transaction> = sdkResponse as Resource<Transaction>;

  if (!castedSdkResponse.resource) {
    throw createAndLogError( THERE_IS_A_PROBLEM, `Transaction API POST request returned no resource for company number ${companyNumber}`);
  }
...
export const putTransaction = async (session: Session,
  ...
  if (!sdkResponse) {
    throw createAndLogError( THERE_IS_A_PROBLEM, `Transaction API PUT request returned no response for transaction id ${transactionId}, company number ${companyNumber}`);
  }

  if (!sdkResponse.httpStatusCode || sdkResponse.httpStatusCode >= StatusCodes.BAD_REQUEST) {
    throw createAndLogError( THERE_IS_A_PROBLEM, `Http status code ${sdkResponse.httpStatusCode} - Failed to put transaction for transaction id ${transactionId}, company number ${companyNumber}`);
  }
