...
import { closeTransaction } from "../../../services/transaction/transaction_service";
import {
...
export class CheckAnswerHandler extends GenericHandler {
    ...
    async get(req: Request, response: Response): Promise<Object> {
        ...
        if (companyProfile?.type === "llp") {
            this.viewData.statementText =
                "The new email address is an appropriate email address within the meaning given by section 2(5) of the Limited Liability Partnerships Act 2000.";
        } else {
            this.viewData.statementText =
                "The new email address is an appropriate email address as outlined in section 88A(2) of the Companies Act 2006.";
        }
    ...
    async post(req: Request, response: Response): Promise<any> {
        ...
        const acceptAppropriateEmailAddressStatement: boolean | undefined =
            req.body.acceptAppropriateEmailAddressStatement;
        ...
        const apiResponse: ApiResponse<RegisteredEmailAddressCreatedResource> = await postRegisteredEmailAddress(
            session,
            transactionId,
            companyNumber,
            registeredEmailAddress
        ...
        const castedResponse: RegisteredEmailAddressCreatedResource =
            apiResponse.resource as RegisteredEmailAddressCreatedResource;

        await closeTransaction(session, companyNumber, transactionId, castedResponse.id).catch(() => {
