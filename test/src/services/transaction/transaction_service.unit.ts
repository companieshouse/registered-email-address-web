...
describe("transaction service tests", () => {
    ...
    describe("postTransaction tests", () => {
        ...
        it("Should throw an error when no transaction api response", async () => {
            ...
            await expect(postTransaction(session, COMPANY_NUMBER, DESCRIPTION, REFERENCE)).rejects.toBe(undefined);
        });
        ...
        it("Should throw an error when transaction api returns a status greater than 400", async () => {
            ...
            await expect(postTransaction(session, COMPANY_NUMBER, DESCRIPTION, REFERENCE)).rejects.toEqual({
                httpStatusCode: StatusCodes.NOT_FOUND,
            });
        });
    ...
    describe("closeTransaction tests", () => {
        ...
        it("Should throw an error if SERVICE UNAVAILABLE returned from SDK", async () => {
            ...
            await expect(closeTransaction(session, COMPANY_NUMBER, TRANSACTION_ID, OBJECT_ID)).rejects.toEqual({
                httpStatusCode: StatusCodes.SERVICE_UNAVAILABLE,
            });
        });
    });
