...
email_router.get(CHANGE_EMAIL_ADDRESS_URL, async (req: Request, res: Response, next: NextFunction) => {
    ...
    const handler = new ChangeEmailAddressHandler(formValidator, req.session?.data.signin_info?.user_profile?.email);
    await handler
        .get(req, res)
        .then(viewData => {
...
email_router.get(CHECK_ANSWER_URL, async (req: Request, res: Response, next: NextFunction) => {
    await new CheckAnswerHandler().get(req, res).then(viewData => {
...
email_router.post(UPDATE_SUBMITTED, async (req: Request, res: Response, next: NextFunction) => {
    ...
    await handler.post(req, res).then(viewData => {
        const confirmationStatementReturnUrl: string = req.session?.getExtraData(
            CONFIRMATION_STATEMENT_RETURN_URL
        ) as string;
