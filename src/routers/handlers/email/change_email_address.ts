...
import { COMPANY_BASE_URL, CONFIRM_URL, DESCRIPTION, REFERENCE } from "../../../config";
...
export class ChangeEmailAddressHandler extends GenericHandler {
    ...
    async get(req: Request, response: Response): Promise<Object> {
        ...
        if (companyEmailAddress && companyNumber && companyProfile) {
            ...
            const returnToConfirmationStatement: boolean = session.getExtraData(
                RETURN_TO_CONFIRMATION_STATEMENT
            ) as boolean;
            const confirmationStatementReturnUrl: string = session.getExtraData(
                CONFIRMATION_STATEMENT_RETURN_URL
            ) as string;
            ...
            await createTransaction(session, companyNumber)
                .then(transactionId => {
