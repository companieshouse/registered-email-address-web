...
describe("Test ConfirmCompanyHandler", () => {
    ...
    it("GET request to serve company Invalid Company - Get invalid company type page", async () => {
        ...
        request.session?.setExtraData(
            constants.INVALID_COMPANY_REASON,
            validationConstants.INVALID_COMPANY_TYPE_REASON
        );
        await invalidCompanyHandler.get(request, response).then(confirmCompanyResponse => {
            ...
            expect(invalidCompanyResponseJson.pageHeader).toEqual(
                validationConstants.invalidCompanyTypePage.pageHeader
            );
            const pageBody = validationConstants.invalidCompanyTypePage.pageBody.replace(
                new RegExp(validationConstants.COMPANY_NAME_PLACEHOLDER, "g"),
                TEST_COMPANY_NAME
            );
    ...
    it("GET request to serve company Invalid Company - Get invalid company status page", async () => {
        ...
        request.session?.setExtraData(
            constants.INVALID_COMPANY_REASON,
            validationConstants.INVALID_COMPANY_STATUS_REASON
        );
        await invalidCompanyHandler.get(request, response).then(confirmCompanyResponse => {
            const invalidCompanyResponseJson = JSON.parse(JSON.stringify(confirmCompanyResponse));
            expect(invalidCompanyResponseJson.pageHeader).toEqual(
                validationConstants.invalidCompanyStatusPage.pageHeader
            );
            const pageBody = validationConstants.invalidCompanyStatusPage.pageBody.replace(
                new RegExp(validationConstants.COMPANY_NAME_PLACEHOLDER, "g"),
                TEST_COMPANY_NAME
            );
            expect(invalidCompanyResponseJson.pageBody).toEqual(pageBody);
    ...
    it("GET request to serve company Invalid Company - Get company has no exisiting registered email page", async () => {
        ...
        request.session?.setExtraData(
            constants.INVALID_COMPANY_REASON,
            validationConstants.INVALID_COMPANY_NO_EMAIL_REASON
        );
        await invalidCompanyHandler.get(request, response).then(confirmCompanyResponse => {
            const invalidCompanyResponseJson = JSON.parse(JSON.stringify(confirmCompanyResponse));
            expect(invalidCompanyResponseJson.pageHeader).toEqual(
                validationConstants.invalidCompanyNoEmailPage.pageHeader
            );
            const pageBody = validationConstants.invalidCompanyNoEmailPage.pageBody.replace(
                new RegExp(validationConstants.COMPANY_NAME_PLACEHOLDER, "g"),
                TEST_COMPANY_NAME
            );
            expect(invalidCompanyResponseJson.pageBody).toEqual(pageBody);
