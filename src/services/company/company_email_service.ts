...
export const getCompanyEmail = async (companyNumber: string): Promise<Resource<RegisteredEmailAddress>> => {
    ...
    const sdkResponse: Resource<RegisteredEmailAddress> =
        await privateApiClient.registeredEmailAddress.getRegisteredEmailAddress(companyNumber);
    ...
    if (sdkResponse.httpStatusCode !== StatusCodes.OK) {
        logger.error(
            `Http status code ${sdkResponse.httpStatusCode} - Failed to get registered email address for company number ${companyNumber}`
        );
        return Promise.reject(sdkResponse);
    }
